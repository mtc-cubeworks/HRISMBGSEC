...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "ssscontsummary_detailspreview";
...
$page_layouts["ssscontsummary_detailspreview"] = $layout;
...
$whereClauses = array();
if($mastertable == "contlo")
{
	$formattedValue = make_db_value("FromDate",$_SESSION[$sessionPrefix."_masterkey1"]);
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["ContID"]));
	
	
	//	ContID - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("ContID", $data, $keylink);
			$row["ContID_value"] = $value;
			$format = $pSet->getViewFormat("ContID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("ContID")))
				$class = ' rnr-field-number';
			$row["ContID_class"] = $class;
	//	Employer - 
			$viewContainer->recId = $recordsCounter;
	...
	//	PreparedBy - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("PreparedBy", $data, $keylink);
			$row["PreparedBy_value"] = $value;
			$format = $pSet->getViewFormat("PreparedBy");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("PreparedBy")))
				$class = ' rnr-field-number';
			$row["PreparedBy_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("ssscontsummary", "detailspreview"));
