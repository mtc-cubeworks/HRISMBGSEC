...
require_once("include/earningadjustment_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "earningadjustment_detailspreview";
...
$page_layouts["earningadjustment_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["eaID"]));
	...
	//	AdjustmentDescription - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("AdjustmentDescription", $data, $keylink);
			$row["AdjustmentDescription_value"] = $value;
			$format = $pSet->getViewFormat("AdjustmentDescription");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("AdjustmentDescription")))
				$class = ' rnr-field-number';
			$row["AdjustmentDescription_class"] = $class;
	//	EmployeeID - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("earningadjustment", "detailspreview"));
