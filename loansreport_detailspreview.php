...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "loansreport_detailspreview";
...
$page_layouts["loansreport_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	LoanType - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("LoanType", $data, $keylink);
			$row["LoanType_value"] = $value;
			$format = $pSet->getViewFormat("LoanType");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("LoanType")))
				$class = ' rnr-field-number';
			$row["LoanType_class"] = $class;
	//	FromDate - Short Date
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("loansreport", "detailspreview"));
