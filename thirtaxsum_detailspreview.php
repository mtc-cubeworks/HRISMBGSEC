...
require_once("include/thirtaxsum_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "thirtaxsum_detailspreview";
...
$page_layouts["thirtaxsum_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	13thToDate - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("13thToDate", $data, $keylink);
			$row["13thToDate_value"] = $value;
			$format = $pSet->getViewFormat("13thToDate");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("13thToDate")))
				$class = ' rnr-field-number';
			$row["13thToDate_class"] = $class;
	//	TaxabaleToDate - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("TaxabaleToDate", $data, $keylink);
			$row["TaxabaleToDate_value"] = $value;
			$format = $pSet->getViewFormat("TaxabaleToDate");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("TaxabaleToDate")))
				$class = ' rnr-field-number';
			$row["TaxabaleToDate_class"] = $class;
	//	TaxToDate - Number
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("thirtaxsum", "detailspreview"));
