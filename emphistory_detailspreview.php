...
require_once("include/emphistory_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "emphistory_detailspreview";
...
$page_layouts["emphistory_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["EHID"]));
	
	
	//	EHID - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("EHID", $data, $keylink);
			$row["EHID_value"] = $value;
			$format = $pSet->getViewFormat("EHID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("EHID")))
				$class = ' rnr-field-number';
			$row["EHID_class"] = $class;
	//	DateIn - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("DateIn", $data, $keylink);
			$row["DateIn_value"] = $value;
			$format = $pSet->getViewFormat("DateIn");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("DateIn")))
				$class = ' rnr-field-number';
			$row["DateIn_class"] = $class;
	//	DateOut - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("DateOut", $data, $keylink);
			$row["DateOut_value"] = $value;
			$format = $pSet->getViewFormat("DateOut");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("DateOut")))
				$class = ' rnr-field-number';
			$row["DateOut_class"] = $class;
	//	Division - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("emphistory", "detailspreview"));
