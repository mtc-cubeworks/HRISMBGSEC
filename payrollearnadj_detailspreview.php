...
require_once("include/payrollearnadj_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "payrollearnadj_detailspreview";
...
$page_layouts["payrollearnadj_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["PadjID"]));
	
	
	//	PadjID - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("PadjID", $data, $keylink);
			$row["PadjID_value"] = $value;
			$format = $pSet->getViewFormat("PadjID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("PadjID")))
				$class = ' rnr-field-number';
			$row["PadjID_class"] = $class;
	//	Adjustment - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Adjustment", $data, $keylink);
			$row["Adjustment_value"] = $value;
			$format = $pSet->getViewFormat("Adjustment");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("Adjustment")))
				$class = ' rnr-field-number';
			$row["Adjustment_class"] = $class;
	//	Amount - Number
			$viewContainer->recId = $recordsCounter;
	...
	//	eaID - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("eaID", $data, $keylink);
			$row["eaID_value"] = $value;
			$format = $pSet->getViewFormat("eaID");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("eaID")))
				$class = ' rnr-field-number';
			$row["eaID_class"] = $class;
	//	PSID - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("payrollearnadj", "detailspreview"));
