...
require_once("include/payrolldiv1_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "payrolldiv1_detailspreview";
...
$page_layouts["payrolldiv1_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["PRollID"]));
	
	
	//	PRollID - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("PRollID", $data, $keylink);
			$row["PRollID_value"] = $value;
			$format = $pSet->getViewFormat("PRollID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("PRollID")))
				$class = ' rnr-field-number';
			$row["PRollID_class"] = $class;
	//	Employer - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("payrolldiv1", "detailspreview"));
