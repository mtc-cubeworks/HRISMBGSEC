...
require_once("include/payrolltax_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "payrolltax_detailspreview";
...
$page_layouts["payrolltax_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["PTaxID"]));
	
	
	//	PTaxID - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("PTaxID", $data, $keylink);
			$row["PTaxID_value"] = $value;
			$format = $pSet->getViewFormat("PTaxID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("PTaxID")))
				$class = ' rnr-field-number';
			$row["PTaxID_class"] = $class;
	//	EmployeeID - 
			$viewContainer->recId = $recordsCounter;
	...
	//	Tax - Number
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("Tax")))
				$class = ' rnr-field-number';
			$row["Tax_class"] = $class;
	//	FromDate - Short Date
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("payrolltax", "detailspreview"));
