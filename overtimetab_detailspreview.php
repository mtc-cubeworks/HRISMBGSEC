...
require_once("include/overtimetab_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "default";
		$layout->customCssPageName = "overtimetab_detailspreview";
...
$page_layouts["overtimetab_detailspreview"] = $layout;
...
$whereClauses = array();
if($mastertable == "overtimes")
{
	$formattedValue = make_db_value("OtiD",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("OtiD") . " is null";
	else
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("OtiD") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["OTtabID"]));
	
	
	//	OTtabID - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("OTtabID", $data, $keylink);
			$row["OTtabID_value"] = $value;
			$format = $pSet->getViewFormat("OTtabID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("OTtabID")))
				$class = ' rnr-field-number';
			$row["OTtabID_class"] = $class;
	//	EmployeeID - 
			$viewContainer->recId = $recordsCounter;
	...
	//	OtiD - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("OtiD", $data, $keylink);
			$row["OtiD_value"] = $value;
			$format = $pSet->getViewFormat("OtiD");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
...
$xt->load_template(GetTemplateName("overtimetab", "detailspreview"));
